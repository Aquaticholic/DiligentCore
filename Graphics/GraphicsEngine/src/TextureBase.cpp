...
#define LOG_TEXTURE_ERROR_AND_THROW(...) LOG_ERROR_AND_THROW("Texture '", (Desc.Name ? Desc.Name : ""), "': ", ##__VA_ARGS__)
    ...
    {
        if (!MemInfo.MemorylessTextureBindFlags)
            LOG_TEXTURE_ERROR_AND_THROW("Memoryless attachments is not supported by device");

        if ((Desc.BindFlags & MemInfo.MemorylessTextureBindFlags) != Desc.BindFlags)
            LOG_TEXTURE_ERROR_AND_THROW("BindFlags (", GetBindFlagsString(Desc.BindFlags & ~MemInfo.MemorylessTextureBindFlags), ") is not supported for memoryless texture.");
